...
const ProgramMessages = ({ programId, sails }: Props) => {
  ...
  const toMessages = useMessagesToProgram(
    { destination: programId, source: addressParam, service: serviceName, fn: functionName, from, to },
    isToDirection,
  );

  const fromMessages = useMessagesFromProgram(
    { source: programId, destination: addressParam, service: serviceName, fn: functionName, from, to },
    !isToDirection,
  );
