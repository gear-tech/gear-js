...
import { AppDataSource, Code, Meta, SailsIdl } from '../src/database/index.js';
...
vi.mock('../src/database', async () => {
  const actualModule = await vi.importActual('../src/database');
...
describe('MetaService', () => {
  ...
  beforeEach(() => {
    (AppDataSource.getRepository as ReturnType<typeof vi.fn>).mockImplementation((entity) => {
      switch (entity) {
        case Meta:
          return mockMetaRepo;
        case SailsIdl:
  ...
  describe('Get meta details', () => {
    it('should return meta if found', async () => {
      const params: GetMetaParams = { hash: 'existingTestHash', hex: '0x' };
    ...
    it('should throw MetaNotFoundError if meta not found', async () => {
      const params: GetMetaParams = { hash: 'testHash', hex: '0x' };
