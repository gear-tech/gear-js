...
const VoucherTableDeprecated = withAccount(({ programId }: Props) => {
  const { isVoucherReady, isVoucherExists, voucherBalance } = useAccountVoucherDeprecated(programId);
  ...
  return isVoucherReady ? (
    <Table>
      ...
      <TableRow name="Amount">
        <span className={styles.highlight}>{balance?.value}</span>
