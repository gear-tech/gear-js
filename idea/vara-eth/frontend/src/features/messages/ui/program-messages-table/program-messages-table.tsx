...
const INCOMING_COLUMNS = [
  {
    ...
    render: (messageId: string) => (
      <HashLink
        ...
        truncateSize="sm"
        maxLength={8}
        href={generatePath(routes.message, { messageId })}
      />
    ),
  },
  {
...
const ProgramMessagesTable = ({ programId, pageSize = 5 }: Props) => {
  ...
  const incoming = useGetAllMessageRequestsQuery(
    incomingPage,
    pageSize,
    programId
  );
  const outgoing = useGetAllMessageSentsQuery(
    outgoingPage,
    pageSize,
    programId
  );

  const incomingData = incoming.data?.data.map((item) => ({
    id: item.id,
    messageId: item.id,
  ...
  const outgoingData = outgoing.data?.data.map((item) => ({
    id: item.id,
    messageId: item.id,
        ...
        <div className={styles.filterButtons}>
          {FILTERS.map((label, i) => (
            <button
              ...
              className={clsx(
                styles.filterButton,
                i === filterIndex && styles.filterButtonActive
              )}
