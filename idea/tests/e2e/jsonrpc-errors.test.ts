...
describe('jsonrpc errors', () => {
  test(JSONRPC_ERRORS.MethodNotFound.name, async () => {
    const response = await request('invalid.method', {});
  ...
  test(JSONRPC_ERRORS.InvalidParams.name, async () => {
    const response = await invalidRequest('program.all', {});
  ...
  test(JSONRPC_ERRORS.UnknownNetwork.name, async () => {
    const response = await invalidRequest('program.all', { genesis: '0x1234' });
    expect(response).toHaveProperty('error.code', -32605);
    expect(response).toHaveProperty('error.message', 'Unkown network');
  });

  test(JSONRPC_ERRORS.ProgramNotFound.name, async () => {
    const response = await invalidRequest('program.data', { genesis, id: '0x1234' });
