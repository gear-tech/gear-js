...
import { IMessageSpec, IProgramSpec, IPrepared, ICodeSpec, IPreparedVara } from '../interfaces';
...
export async function processPrepareVara(api: GearApi): Promise<IPreparedVara> {
  const programs = load(readFileSync('./spec/programs.yaml', 'utf8')) as { [program: string]: IProgramSpec };
  const [ uploadedPrograms ] = await uploadPrograms(api, programs);

  return { programs: uploadedPrograms };
}
