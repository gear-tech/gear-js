...
const FilesPanel = ({ files, openedFiles, handleFileClose, handleFileSelect, currentFile }: Props) => {
    ...
    return (
        <div className="files-panel">
            {
                files && files.length && files.map((curFile: EditorFile, index: number) => {
                    if (openedFiles.includes(index)) {
                        ...
                        return (
                            <button className={clsx('files-panel--item', currentFile === index && 'is-active')} type="button" onClick={(event: React.MouseEvent) => {
                                ...
                                {curFile.name}
                                <button className="files-panel--item__close" type="button" onClick={(event: React.MouseEvent) => {
                                    if (fileRef && fileRef.current?.contains(event.target as Node)) {
