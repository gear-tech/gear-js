...
export function getTypesFromTypeDef(
  ...
  const namespaces = withNamespaces ? new Map<string, string>() : undefined;
  ...
  portableReg.types.forEach(({ id, type: { path } }) => {
    ...
    typesFromTypeDef[typeDef.lookupName || typeDef.lookupNameRoot] = typeDef.type;
  ...
  return withNamespaces ? { typesFromTypeDef, namespaces } : { typesFromTypeDef };
}
