...
const getLegacyPayloadSchema = (sails: Sails, args: ISailsFuncArg[], encode: (..._args: unknown[]) => HexString) => {
  ...
  const result = args.map(({ typeDef }, index) => [index, getSchema(typeDef as ISailsTypeDef)] as const);
