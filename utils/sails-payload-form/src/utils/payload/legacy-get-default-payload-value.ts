...
const getLegacyDefaultValue = (sails: Sails) => {
...
const getLegacyDefaultPayloadValue = (sails: Sails, args: ISailsFuncArg[]) => {
  const result = args.map(
    ({ typeDef }, index) => [index, getLegacyDefaultValue(sails)(typeDef as ISailsTypeDef)] as const,
  );

  return Object.fromEntries(result);
};

export { getLegacyDefaultPayloadValue, getLegacyDefaultValue };
