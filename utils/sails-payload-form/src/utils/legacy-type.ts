...
const getLegacyType = (def: ISailsTypeDef): string => {
  if (def.isPrimitive) return getPrimitiveType(def.asPrimitive);
  if (def.isOptional) return `Option<${getLegacyType(def.asOptional.def)}>`;
  if (def.isResult) return `Result<${getLegacyType(def.asResult.ok.def)}, ${getLegacyType(def.asResult.err.def)}>`;
  if (def.isVec) return `Vec<${getLegacyType(def.asVec.def)}>`;
  if (def.isFixedSizeArray) return `[${getLegacyType(def.asFixedSizeArray.def)}; ${def.asFixedSizeArray.len}]`;
  if (def.isMap) return `BTreeMap<${getLegacyType(def.asMap.key.def)}, ${getLegacyType(def.asMap.value.def)}>`;
  if (def.isUserDefined) return def.asUserDefined.name;

  if (def.isStruct) {
    if (def.asStruct.isTuple) return `(${def.asStruct.fields.map((field) => getLegacyType(field.def)).join(', ')})`;

    const result = def.asStruct.fields.map((field) => [field.name, getLegacyType(field.def)]);
  ...
  if (def.isEnum) {
    ...
    const result = def.asEnum.variants.map((variant) => [
      variant.name,
      variant.def ? getLegacyType(variant.def) : 'Null',
    ]);
