...
ANVIL_DIR=/tmp/vara-eth/anvil
VARAETH_DIR=/tmp/vara-eth/vara-eth
GEAR_REPO_DIR=/tmp/vara-eth/gear
...
cleanup() {
    ...
    if [[ -n "$VARAETH_PID" ]]; then
        log_info "Stopping Gear node (PID: $VARAETH_PID)..."
        kill $VARAETH_PID 2>/dev/null || true
...
build_varaeth() {
    log_info "Building varaeth..."
    cd "$GEAR_REPO_DIR"
    log_info "Running cargo build for ethexe-cli..."
    if ! cargo build -p ethexe-cli --release; then
        log_error "Failed to build varaeth"
        exit 1
    fi
    log_success "Successfully built varaeth"
}
...
inserted_validator=$(./target/release/ethexe key -k $VARAETH_DIR/keys insert $VALIDATOR_PRIVATE_KEY)
...
inserted_network=$(./target/release/ethexe key -k $VARAETH_DIR/net insert $PRIVATE_KEY)
...
log_info "Starting VaraEth..."
export RUST_LOG=debug
export RUST_BACKTRACE=1

log_info "VaraEth configuration:"
echo "    - Base directory: $VARAETH_DIR"
...
log_info "Launching varaeth node..."
nohup ./target/release/ethexe --cfg none run --tmp --base "$VARAETH_DIR" \
    ...
    --rpc-port 9944 \
    --rpc-cors "all" > $LOGS_DIR/varaeth.log 2>&1 &
VARAETH_PID=$!

log_success "VaraEth node started with PID: $VARAETH_PID"
...
(
  ...
  if [[ $exit_code -ne 0 ]]; then
    log_error "VaraEth exited unexpectedly with code $exit_code"
    log_error "Last 20 lines of varaeth logs:"
    echo "====================================================="
    tail -n 20 $LOGS_DIR/varaeth.log
    echo "====================================================="
    kill $$
  fi
) &
...
if ! kill -0 $VARAETH_PID 2>/dev/null; then
    log_error "VaraEth node failed to start. Check logs at: $LOGS_DIR/varaeth.log"
    log_error "Last 20 lines of varaeth logs:"
    tail -n 20 $LOGS_DIR/varaeth.log
    exit 1
fi
...
touch /tmp/vara-eth/environment-ready
log_success "Ready signal created at /tmp/vara-eth/environment-ready"
