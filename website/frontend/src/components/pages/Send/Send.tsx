...
const Send = () => {
  ...
  const types = useMemo(() => {
    if (metadata && metadata.types && metadata.handle_input) {
      ...
      const typeStructure: MetaItem = createPayloadTypeStructure(metadata.handle_input, decodedTypes);
      setManualStructure(getPreformattedText(createPayloadTypeStructure(metadata.handle_input, decodedTypes, true)));
